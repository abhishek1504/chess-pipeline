...
def main():
    ...
    # Limit to MAX_UPLOADS if set — no point generating more than we can upload
    max_uploads = int(os.environ.get("MAX_UPLOADS", 0))
    if max_uploads > 0 and len(all_wins) > max_uploads:
        print(f"\n⚙️  Limiting to {max_uploads} games (MAX_UPLOADS={max_uploads})")
        print(f"   Skipping {len(all_wins) - max_uploads} games — run again tomorrow for more")
        all_wins = all_wins[:max_uploads]
