...
def find_landscape(game_idx, game_data):
    """Locate the landscape.mp4 for this game.

    Tries the index-named folder first, then falls back to matching by
    player names — step2's folder numbering may be offset from the
    won_games.json index.
    """
    ...
    print(f"❌ No landscape.mp4 found for game {game_idx} "
          f"({white} vs {black}) — run step2 first!")
    sys.exit(1)
...
def main():
    ...
    def add_freeze(at, dur):
        ...
        # Seeking too close to EOF makes ffmpeg silently emit nothing — clamp,
        # and if extraction still comes back empty, grab 1s before end-of-file.
        at = min(at, total_orig - 1.0)
        subprocess.run(["ffmpeg", "-y", "-ss", str(at), "-i", src,
                        "-frames:v", "1", png], capture_output=True)
        if not os.path.exists(png):
            run(["ffmpeg", "-y", "-sseof", "-1", "-i", src,
                 "-frames:v", "1", png], f"frame @EOF")
        p = os.path.join(tmp, f"seg_{len(seg_paths):02d}.mp4")
